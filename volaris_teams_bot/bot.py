...
from router import InMemorySessionStore, is_affirmation_or_short, remember_user, set_sticky
...
def is_short_followup(user_text: str) -> bool:
    s = (user_text or "").strip().lower()
...
class VolarisBot(ActivityHandler):
    def __init__(self, store: InMemorySessionStore):
        self.store = store
        self.use_router_structured = os.getenv("ROUTER_STRUCTURED", "true").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        if route == "conversacional":
            ...
            await safe_send_text(turn_context, answer, logger=LOGGER)
            return
        ...
        if route == "politicas":
            pol_tid = (
                session.politicas_thread_id
                or session.threads_by_domain.get("politicas")
                or session.prev_politicas
            )
            ...
            gas_tid = (
                session.gastos_thread_id
                or session.threads_by_domain.get("gastos")
                or session.prev_gastos
            )
